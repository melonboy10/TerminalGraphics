# Recursively get all .h file in src
SRC := $(shell find src -name '*.h')

# Run ./split_file.sh on all .h files and pass in the path to the file and the ./out directory
split_file:
	@for file in $(SRC); do \
		./split_file.sh $$file ./out; \
	done
...
# SRCS := $(patsubst $(SRCDIR)/%,%,$(call rwildcard,$(SRCDIR),*.cpp))
...
# $(OBJDIR)/%.o : $(SRCDIR)/%.cpp $(DEPDIR)/%.d | $(SUBDIRECTORIES)
